...
const TurtleList = () => {
  ...
  return (
    <aside className="w-72 bg-card rounded-lg shadow-panel p-4 flex flex-col gap-4">
      ...
      <div className="flex-1 overflow-y-auto flex flex-col gap-3">
        {turtles.length === 0 ? (
          <div className="text-center text-gray-500 py-8">No turtles connected</div>
        ) : (
          turtles.map((turtle) => (
            <div
              key={turtle.id}
              className={`bg-background rounded-md p-3 cursor-pointer border-2 transition hover:-translate-y-0.5 hover:shadow-sm
                ${selectedTurtle === turtle.id ? 'border-primary' : 'border-transparent'}`}
